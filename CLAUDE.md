...
- **Extract reusable components**: When a component file contains multiple sub-components (like `category-input`, `condition-row-ui`, `action-row-ui` in a dialog), consider extracting them into separate files
...
## Linting
- After any code changes, run `pnpm --filter fin-web check:write` from the project root, or `pnpm check:write` from `apps/web/` to ensure code is in line with linting rules
...
All API handlers in `apps/api/src/server/handlers/` must use wrapper functions from `handler-wrappers.ts` instead of manual auth checks. Use `withAuth` for handlers that require real authentication (mutations, user-specific writes) and `withPublic` for unauthenticated endpoints. These wrappers guarantee `userId` is a non-null string in the handler context. The wrapper resolves the correct user ID before the handler runs, providing type safety and consistent auth behavior across all endpoints.
