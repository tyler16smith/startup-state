		...
		<AnimatePresence>
			{open ? (
				<motion.aside
					...
					className="absolute top-32 left-4 z-30 flex max-h-[calc(100%-9rem)] w-[min(24rem,calc(100%-2rem))] flex-col overflow-hidden rounded-lg border border-white/70 bg-white/90 shadow-2xl backdrop-blur-md md:top-20"
					exit={{ opacity: 0, y: -12 }}
				...
				>
					<div className="flex items-start justify-between gap-3 border-white/60 border-b px-4 py-3">
						...
						<button
							...
							className="fade-in-out flex size-8 shrink-0 items-center justify-center rounded-full text-slate-500 transition-all duration-300 hover:bg-gray-200 hover:text-slate-950"
							onClick={onClose}
							type="button"
						>
							<X className="size-4" />
